...
class Copydata(object):
    ...
    def __init__(self, src, localdrive, project, filetype, currentFileName, shotGeneric, c2, c3, c4, fileext):
        ...
        :param currentFileName: selected file name
        ...
        if status[0][0] == 'CLOSED' and (status[0][1] == 'NYS' or status[0][1] == 'TWIP' or status[0][1] == 'CWIP'):
            ...
            if self.latest_file != None:
                ...
                if status[0][1] == 'NYS':
                    ...
                    # subprocess.Popen("C:\\Program Files\\Autodesk\\Maya2015\\bin\\maya.exe self.dst")
                    msgbox.setText('File has been copied to local folder')
                    ret = msgbox.exec_()
                else:
                    ...
                    # self.conn.commit()
                    # print "UPDATE `"+self.sequence+"` SET `" + self.dept + "_status`='TWIP',`Session_status`='OPEN' WHERE `shots`='"+self.shot+"'"
                    # print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ copy done in if-upversion for current version"
            else:
